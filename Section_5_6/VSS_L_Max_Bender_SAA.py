...
def get_co (slope, e_F, e_Fprime, xi_F, xi_Fprime, w):
    # global shortage because of export bans
    G = sum(e_Fprime[k]*(1-xi_Fprime[k,w]) + e_F[k]*(1-xi_F[k,w]) for k in K)  
...
m_subp.setObjective(gp.quicksum(gp.quicksum((c_s[k]-c_pr[j]-c_t2[j,k])*V[j,k] for k in K) for j in J)+ 
                    gp.quicksum(c_o*xi_Fprime[j,0]*V[j,j] + gp.quicksum(c_o*V[j,k] for k in K if k!=j) for j in J)-
...
for i in I:
    c2_1[i] = m_subp.addConstr(gp.quicksum(U[i,j] for j in J) <= q_sc[i]*xi_sc[i,0])
    
# raw material purchases - supplier capacity and export ban for non-allies      
for i in I:
    for j in JnFprime[i]:
        c2_2[i,j] = m_subp.addConstr(U[i,j] <= q_sc[i]*xi_sc[i,0]*xi_Fprime[i,0])    

# raw material purchases - supplier capacity and export ban for allies     
for i in I:
    for j in JnF[i]:
        c2_3[i,j] = m_subp.addConstr(U[i,j] <= q_sc[i]*xi_sc[i,0]*xi_F[i,0]) 

# production - manufacturing capacity
for j in J:
    c2_4[j] = m_subp.addConstr(gp.quicksum(V[j,k] for k in K) <= q_pc[j]*xi_pc[j,0])
    
# production - manufacturing capacity and export ban for non-allies      
for j in J:
    for k in Fprime[j]:
        c2_5[j,k] = m_subp.addConstr(V[j,k] <= q_pc[j]*xi_pc[j,0]*xi_Fprime[j,0])    
    
# production - manufacturing capacity and export ban for allies     
for j in J:
    for k in F[j]:
        c2_6[j,k] = m_subp.addConstr(V[j,k] <= q_pc[j]*xi_pc[j,0]*xi_F[j,0])  
           
# demand constraint
for k in K:
    c2_7[k] = m_subp.addConstr(gp.quicksum(V[j,k]for j in J)+S[k] == xi_dfinal[k,0])
...
def f_rep(m):
    ...
    for w in range(W):
        ...
        for i in I:
            sprobabilities = [supplier_df2.loc[i, c] for c in cap_ava]          
            xi_ss[i,w] = np.sum(np.array(cap_ava)*np.array(sprobabilities))     # strains supplier capacity
            xi_sd[i,w] = p_sd[i][0]                                             # disruption of supplier capacity (1 operating, 0 o/w)
            xi_sc[i,w] = round(xi_ss[i,w]*xi_sd[i,w],4)                         # supplier capacity available
            xi_tsc += q_sc[i]*xi_sc[i,w]   
          
        for j in J:
            pprobabilities = [plant_df2.loc[j, c] for c in cap_ava]             
            xi_ps[j,w] = np.sum(np.array(cap_ava)*np.array(pprobabilities))     # strains in production capacity
            xi_pd[j,w] = p_pd[j][0]                                             # disruption of production capacity (1 operating, 0 o/w)
            xi_pc[j,w] = round(xi_ps[j,w]*xi_pd[j,w],4)                         # production capacity available
          
        
        for k in K:
            xi_Fprime[k,w] = round((p_prime[k][0]*0.14756)+(1-0.14756),4)                               # Expected
            xi_F[k,w] = round(0.14756*(p[k][0]+p_prime[k][0]-(p[k][0]*p_prime[k][0]))+(1-0.14756),4)    # Expected
            xi_d[k,w] = d[k][0]                                                                         # Expected
            xi_dfinal[k,w] = max(xi_d[k,w]-(1-xi_Fprime[k,w])*e_Fprime[k]-(1-xi_F[k,w])*e_F[k], 0)
     #-------------------------------------------------------------------------------------------------------------------------------------------------------
