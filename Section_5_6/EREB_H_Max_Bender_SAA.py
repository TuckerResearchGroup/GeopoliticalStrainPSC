...
def f_rep(m):
    ...
    # Case expected exports bans in the replication phase
    for w in range(W): 
        for k in K:
            xi_Fprime[k,w] = round((p_prime[k][0]*0.14756)+(1-0.14756),4)
            xi_F[k,w] = round(0.14756*(p[k][0]+p_prime[k][0]-(p[k][0]*p_prime[k][0]))+(1-0.14756),4)
            xi_dfinal[k,w] = max(xi_d[k,w]-(1-xi_Fprime[k,w])*e_Fprime[k]-(1-xi_F[k,w])*e_F[k], 0)
