...
def gen_scenarios(d, total_sc, supplier_df2, cap_ava, plant_df2, p, p_prime, p_sd, p_pd, W):              # Sampling, Random parameters
    ...
    for w in range(W):        
        ...
        if (xi_tsc/total_sc) < 0.8:                                         # if sc is less than 80%, then there is export risk, conditional part
            for k in K:            
                xi_Fprime[k,w] = 1                                          # export risk for non-allies
...
meanS, meanPS, meanV, meanU = {}, {}, {}, {}                # statistics 
...
if __name__=='__main__':
    ...
    for w in range(numE):          
        ...
        for k in K:
            ...
            GD[w] += xi_dfinal[k,w]                             # global demand per scenario considering lost demand
            GRD[w] += xi_d[k,w]                                 # global actual demand per scenario    
            
            SE[k,w] = S[k].x                                    # shortage
            PSE[k,w] = S[k].x/(xi_dfinal[k,w]+0.0000001)        # percentage of shortage
            PSRE[k,w] = S[k].x/(xi_d[k,w]+0.0000001)            # percentage of shortage
    ...
    PGS = np.zeros(numE)
    for w in range(numE):
        PGS[w] = GS[w]/GD[w]                                                # percentage of global shortage in every scenario   
    ...
    shortage = pd.DataFrame(columns=["ISO","meanS","semS","CIS","meanPS","semPS","CIPS"])
    for k in K:
        shortage = pd.concat([shortage, pd.DataFrame.from_records([{"ISO":k,"meanS":meanS[k],"semS":semS[k],"CIS":CIS[k],
                                             "meanPS":meanPS[k],"semPS":semPS[k],"CIPS":CIPS[k]}])], ignore_index=True)
    ...
    scenarios_grd.to_excel(writer,"PRD")
